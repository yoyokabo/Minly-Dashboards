...
export const createUser: RequestHandler<unknown, unknown, CreateUserBody, unknown> = async (req,res,next) => {
    try {
        ...
        new Promise((resolve,reject) => bcrypt.hash(req.body.password, saltRounds, (err: unknown, hash: unknown) => {
            if(err){
...
export const authUser: RequestHandler<unknown, unknown, AuthUserBody, unknown> = async (req,res,next) => {
    try {
        if (!req.body.password) {
            throw createHttpError(400, "Password Invalid");
        }
        ...
        if(await bcrypt.compare(req.body.password,user.passwordHash)){
            const token = jwt.sign({_id: user._id?.toString(), name: user.username}, secretKey, {
                expiresIn: '1h'
            });
