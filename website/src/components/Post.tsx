...
const Post = ({ post, className }: PostProps) => {
  let { username, _id, likes, caption, createdAt, mediatype } = post;
  ...
  if (token) {   // Refactor into utils folder
  ...
  return (
    <Card className={`${styles.postCard} ${className}`}>
      <Card.Body className={styles.cardBody}>
        <Card.Title className={styles.postText}>{username}</Card.Title>
        {mediatype === "image" && (
          <Card.Img src={`api/media/${_id}`} className={styles.cardContent} />
        )}
        {mediatype === "video" && (
        ...
        <Card.Title className={styles.postText}>{caption}</Card.Title>
        ...
        <Card.Footer className={styles.postText}>{date}</Card.Footer>
      </Card.Body>
    </Card>
  );
};
